import Pkg
using JuMP, AmplNLWriter, Ipopt, NEOSServer
...
open(name_file, "w") do Output
	for NUM_PIXEL in 1:1
		...
		while (status != "LOCALLY_SOLVED") & (status != "OPTIMAL") & (status != "TIME_LIMIT") & (status != "OTHER_LIMIT") & (NUM_SATELLITE < NUM_PIXEL*PERIOD)
			...
			# Ho lavorato sul MINLP routine e credo che il problema sia Ipopt chiamato dal solver globale che sia Couenne o Juniper… Io credo che il modello 
			# abbia seri problemi numerici. Faccio ancora qualche tests, ma penso che la situazione non migliorerà. Se ci dovessero eventualmente chiedere più’
			# dettagli sulla soluzione del MINLP, secondo me, ci basta dire appunto che Ipopt ha seri errori numerici (cosa che d’altra parte ho riscontrato 
			# anche quando codavo il FP). Per evitare questi problemi, ho fatto risolvere ad Ipopt lo stesso problema da diversi punti iniziali scelti random…

			# sostituire la funzione % con il vincolo Longitude in [0,2pi] non e' corretto. Couenne e Alpine non supportano la funzione %. 
			# Juniper funziona, ma risolve problemi convessi e quindi nel nonsto caso non e' detto restituisca l'ottimo globale (ti allego comunque 
			# il codice che almeno compila e non restituice INFEASIBLE)
			# https://crypto.stackexchange.com/questions/100419/can-a-modulo-function-be-linearized-or-alternatively-expressed
			# https://math.stackexchange.com/questions/2513559/how-to-model-a-modulo-with-linear-constraints
			
			# Bisogna per forza usare Couenne perche' Alpine non ammette ancora variabili intere non binarie.
			...
			# model = Model() do 
				# NEOSServer.Optimizer(email="julien.floquet@protonmail.com", solver="OCTERACT")
			# end
			# model = Model(() -> AmplNLWriter.Optimizer("D:/jfloquet/Octeract/bin/octeract-engine"))
