...
global name_file = "MILP_discretize_"*string(numbOfDiscretize)*"_theta_"*string(round(Theta_min*180/pi))*"_Tsimu"*string(Int(TEMPS_SIMU/3600))*"H_DeltaT"*string(Int(DELTA_T/3600))*"H_dt"*string(Int(dt/60))*"Mn.txt" 
...
open(name_file, "w") do Output
	for NUM_PIXEL in RANGE_NUM_PIXEL
		...
		@variable(model, inclinaison[1:NUM_SATELLITE], start = start_variable)
		@variable(model, activation_inclinaison[1:NUM_SATELLITE,1:n_inclinaison], Bin, start = start_variable)	
		
		# Noeud ascendant
		@variable(model, noeudAscendant[1:NUM_SATELLITE], start = start_variable)
		...
		@variable(model, meanAnomaly[1:NUM_SATELLITE], start = start_variable)
		...
		@variable(model, zeta[1:NUM_SATELLITE], Bin, start = start_variable)
		@variable(model, chi[1:NUM_SATELLITE], Bin, start = start_variable)
		
		# Fonction objectif : Minimize the number of active satellite (choice between the next 2:)
		# Objectif de minimiser le nombre de sat
		...
		@constraint(model, [p=1:NUM_TIME,j=1:NUM_PIXEL,i=1:NUM_SATELLITE], Thetamax[i]+(1-Xi[i,p,j])*M_limit >= (sum(act4[i,a,k,l,s]*CoverageSatLat[j,p,s,l,k,a]
		for s=1:n_inclinaison for l=1:n_noeudAscendant for k in 1:n_meanAnomaly for a=1:length(altitudeSet))))
		@constraint(model, [p=1:NUM_TIME,j=1:NUM_PIXEL,i=1:NUM_SATELLITE], Thetamax[i]+(1-Xi[i,p,j])*M_limit >= (sum(act4[i,a,k,l,s]*(-1)*CoverageSatLat[j,p,s,l,k,a]
		for s=1:n_inclinaison for l=1:n_noeudAscendant for k in 1:n_meanAnomaly for a=1:length(altitudeSet))))
		@constraint(model, [p=1:NUM_TIME,j=1:NUM_PIXEL,i=1:NUM_SATELLITE], Thetamax[i]+(1-Xi[i,p,j])*M_limit >= (sum(act4[i,a,k,l,s]*CoverageSatLong[j,p,s,l,k,a]
		for s=1:n_inclinaison for l=1:n_noeudAscendant for k in 1:n_meanAnomaly for a=1:length(altitudeSet))))
		@constraint(model, [p=1:NUM_TIME,j=1:NUM_PIXEL,i=1:NUM_SATELLITE], Thetamax[i]+(1-Xi[i,p,j])*M_limit >= (sum(act4[i,a,k,l,s]*(-1)*CoverageSatLong[j,p,s,l,k,a]
		for s=1:n_inclinaison for l=1:n_noeudAscendant for k in 1:n_meanAnomaly for a=1:length(altitudeSet))))
		...
		@constraint(model, [i=1:NUM_SATELLITE], zeta[i] <= 1)
		@constraint(model, [i=1:NUM_SATELLITE], zeta[i] <= sum(Xi[i,p,j] for p in 1:NUM_TIME for j in 1:NUM_PIXEL))
		@constraint(model, [i=1:NUM_SATELLITE], chi[i] => {zeta[i] >= sum(Xi[i,p,j] for p in 1:NUM_TIME for j in 1:NUM_PIXEL)})
		@constraint(model, [i=1:NUM_SATELLITE], !chi[i] => {zeta[i] >= 1})
		@constraint(model, [i=1:NUM_SATELLITE], chi[i] => {sum(Xi[i,p,j] for p in 1:NUM_TIME for j in 1:NUM_PIXEL) <= 1})
		@constraint(model, [i=1:NUM_SATELLITE], !chi[i] => {sum(Xi[i,p,j] for p in 1:NUM_TIME for j in 1:NUM_PIXEL) >= 1})	
		...
		if string(status) == "LOCALLY_SOLVED" || string(status) == "OPTIMAL"
			...
			write(Output,"\n  k =  ",string(findall(a->a==1,value.(model[:activation_meanAnomaly])[:,:][1])))
			write(Output,"\n  l =  ",string(findall(a->a==1,value.(model[:activation_noeudAscendant])[:,:][1])))
			write(Output,"\n  s =  ",string(findall(a->a==1,value.(model[:activation_inclinaison])[:,:][1])))
			write(Output,"\n"," ")
